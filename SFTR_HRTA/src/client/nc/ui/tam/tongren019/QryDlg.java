...
public class QryDlg extends QryDlg2{
	...
	protected void initRef(CriteriaChangedEvent event){
		if("pk_bb".equals(event.getFieldCode())){
			...
			String wheredept = 	" pk_corp='" + pk_corp+ "' and isnull(bisseal,'N')='N' and pk_dept in (select pk_docid from bd_tr_userpower where isnull(dr,0)=0 and pk_user='"+operatorid+"' and powertype=0 and  pk_corp='"+pk_corp+"') ";
