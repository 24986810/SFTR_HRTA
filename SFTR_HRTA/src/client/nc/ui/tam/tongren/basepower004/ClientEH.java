...
public class ClientEH extends TreeCardEventHandler {
	...
	protected void onBoSave() throws Exception {
		...
		ArrayList<UserClassDeptVO> list = new ArrayList<UserClassDeptVO>();
		
		for(int i = 0 ; i < cnt ; i++){
			Object temp = getBillCardPanelWrapper().getBillCardPanel().getBodyValueAt(i, "flag"); 
			if(temp != null && temp.toString().equals("true")){
				String btt = getBillCardPanelWrapper().getBillCardPanel().getBodyValueAt(i, "pk_dept").toString();
				UserClassDeptVO ucavo = new UserClassDeptVO();
		...
		irewrite.setClassRoleOrUserAtdTypeAuth(tempid,list,vflag,pk_module,_getCorp().getPrimaryKey(),0);
