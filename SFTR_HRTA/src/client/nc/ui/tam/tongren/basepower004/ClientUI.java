...
//			 RefModule.setPreferredSize(new Dimension(100,20));
	...
	public SuperVO[] getBodysVO() {
		// TODO 自动生成方法存根
		try {
			return HYPubBO_Client.queryByCondition(DeptKqVO.class, " isnull(dr,0)=0 and pk_corp='"+_getCorp().getPrimaryKey()+"' and isnull(bisseal,'N')='N' order by nvl(pk_parent,pk_dept),vcode ");
	...
	protected void onTreeSelectSetButtonState(TableTreeNode arg0) {
		...
		try{
			...
			UserClassDeptVO[] authVOs = (UserClassDeptVO[])business.queryByCondition(UserClassDeptVO.class,sql);
			
			int nflag = this.getBillCardPanel().getRowCount();
			for (int i = 0; i < nflag; i++) {
				this.getBillCardPanel().setBodyValueAt(false, i, "flag");
				String code = this.getBillCardPanel().getBodyValueAt(i,"pk_dept").toString();
