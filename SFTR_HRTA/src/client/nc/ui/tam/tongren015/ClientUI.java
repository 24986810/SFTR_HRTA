...
public class ClientUI extends HRPManagerSingleHeadUI implements BillEditListener{
	public ClientUI() {
		...
		getButtonManager().getButton(IBillButton.Audit).setName("确认");
		getButtonManager().getButton(IBillButton.CancelAudit).setName("取消确认");
		updateButtons();
		getBillListPanel().setEnabled(true);				//列表可编辑
	...
	public boolean beforeEdit(BillEditEvent e) {
		...
		if(isselect!=null&&new UFBoolean(isselect.toString()).booleanValue()){
			if(getBillListPanel().getHeadBillModel().getValueAt(e.getRow(), "istate")!=null
					&&getBillListPanel().getHeadBillModel().getValueAt(e.getRow(), "istate").toString().equals("提交")){
