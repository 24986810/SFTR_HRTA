 ...
 *
 */
public class EventHandler3 extends CardEventHandler {
	 ...
	 * @param control
	 */
	public EventHandler3(BillCardUI billUI, ICardController control) {
	...
	/*	
		//20190805
			...
			for(PaibanWeekVO weekvo:weekvos){
				//weekvo.setVbillstatus3("0");//lzch insert
				for(int i=1;i<31;i++){
...
//		 zhanghua
		...
		((ClientUI3)getBillUI()).setEditstate(1);
...
//		UFDate end = new UFDate(vdate.substring(11));
		...
		try {
			...
			HashMap<String,PaibanTempVO> map_0805 = new HashMap<String, PaibanTempVO>();
			PaibanTempVO[] tvos = (PaibanTempVO[])HYPubBO_Client.queryByCondition(PaibanTempVO.class, " isnull(dr,0)=0 and pk_corp='"+_getCorp().getPrimaryKey()+"' ");
			
			if(list_vo !=null&& list_vo.size()>0){
				for(PaibanTempVO vo:tvos){
					map_0805.put(vo.getPk_psndoc(), vo);
				}
			}
			...
			for(PaibanWeekVO weekvo:weekvos){
				...
				PaibanTempVO tempvo = map_0805.get(weekvo.getPk_psndoc());
				if(tempvo!=null){
					tempvo.setNnowdays(weekvo.getNnowdays());
				}
				list_psn.add(weekvo.getPk_psndoc());
			...
			String begin_0805 = getBillCardPanelWrapper().getBillCardPanel().getHeadItem("begindate").getValueObject().toString();
			PanbanWeekBVO[] bvos_0805 =(PanbanWeekBVO[]) HYPubBO_Client.queryByCondition(PanbanWeekBVO.class, " isnull(dr,0)=0 "+HRPPubTool.formInSQL("pk_psndoc", list_psn)+" and pk_bb in ('10028L100000000002D8','10028L100000000002XL') and ddate like '"+begin1.getYear()+"%' ");
			HashMap<String,UFDouble> map_gxs = new HashMap<String, UFDouble>();
			
			if(bvos_0805!=null&&bvos_0805.length>0){
				for(PanbanWeekBVO bvo:bvos_0805){
					String pk_bb = bvo.getPk_bb();
					UFDouble gxs = map_gxs.get(bvo.getPk_psndoc())!=null?map_gxs.get(bvo.getPk_psndoc()):new UFDouble(0);
					if(pk_bb.equals("10028L100000000002D8")){
						gxs = gxs.add(1);
				...
				GxBVO[] gxvos_0805 = (GxBVO[])HYPubBO_Client.queryByCondition(GxBVO.class,
						" isnull(dr,0)=0 and vyear='"+new UFDate(begin_0805).getYear()+"' and pk_corp='"+_getCorp().getPrimaryKey()+"' "+HRPPubTool.formInSQL("pk_psndoc", list_psn)+" ");
				if(gxvos_0805!=null&&gxvos_0805.length>0){
					for(GxBVO gxvo:gxvos_0805){
						gxvo.setNlastsygx(gxvo.getNsxgx());
						gxvo.setNlastyygx(gxvo.getNyygx());
						gxvo.setNyygx(map_gxs.get(gxvo.getPk_psndoc()));
						gxvo.setNsxgx((gxvo.getNsfgx()!=null?gxvo.getNsfgx():new UFDouble(0)).sub(gxvo.getNyygx()!=null?gxvo.getNyygx():new UFDouble(0)));
				...
				HYPubBO_Client.updateAry(gxvos_0805);
			}else{
				GxBVO[] gxvos_0805 = (GxBVO[])HYPubBO_Client.queryByCondition(GxBVO.class,
						" isnull(dr,0)=0 and vyear='"+new UFDate(begin_0805).getYear()+"' and pk_corp='"+_getCorp().getPrimaryKey()+"' "+HRPPubTool.formInSQL("pk_psndoc", list_psn)+" ");
				if(gxvos_0805!=null&&gxvos_0805.length>0){
...
//		}
	...
	protected void onBoCopy() throws Exception {
		// TODO Auto-generated method stub
		int x = MessageDialog.showOkCancelDlg(this.getBillUI(), "提示", "确认按上月数据当前排班数据?");
		if(x!=UIDialog.ID_OK) return;
		String vdate =  getBillCardPanelWrapper().getBillCardPanel().getHeadItem("vdate").getValueObject().toString();
		vdate =  vdate.substring(0,10);
		int lastMonthDay = calculateLastMonthDay(vdate);
		int thisNonthDay = calculateThisMonthDay(vdate);
		
		UFDate begin = new UFDate(vdate).getDateBefore(lastMonthDay);
	...
	public void setdate(){
		...
		int day = calculateThisMonthDay(vdate.substring(0,10));
		
		for(int i=1;i<=31;i++){
			//String name = getBillCardPanelWrapper().getBillCardPanel().getBillModel().getItemByKey("vbbname"+(i)+"").getName().substring(0,3); 星期几
			int week = begin.getDateAfter(i-1).getWeek();
			String name = "";
			if(week==0){
				name = "星期日";
	...
	protected void onBoElse(int intBtn) throws Exception {
		...
		case 801://下月
			...
			int nextMonthDay = calculateNextMonthDay(vdate);
			UFDate end = new UFDate(vdate).getDateAfter(nextMonthDay);
			getBillCardPanelWrapper().getBillCardPanel().getHeadItem("vdate").setValue(begin+"至"+end);
		...
		case 802:  //上月
			...
			int monthDay = calculateLastMonthDay(vdate);
			
			begin = new UFDate(vdate).getDateBefore(monthDay);
			...
			break;
		case 803://本月
			//int x = _getDate().getWeek(); //表示周几
			int month = _getDate().getMonth();
			 int day = _getDate().getDay();
			 int year = _getDate().getYear();
			 vdate = new Integer(year).toString()+"-"+new Integer(month).toString()+"-01";
			 
			 //vdate =  getBillCardPanelWrapper().getBillCardPanel().getHeadItem("vdate").getValueObject().toString();
			 //vdate =  vdate.substring(0,10);
			 
			 int thisMonthDay = calculateThisMonthDay(vdate);
			begin = _getDate().getDateBefore(day-1);
			end = _getDate().getDateAfter(thisMonthDay-day);
...
public int calculateLastMonthDay(String vdate){
	int month = new UFDate(vdate).getMonth()-1;
	if(month==0){
		month=12;
	}
...
public int calculateNextMonthDay(String vdate){
	int month = new UFDate(vdate).getMonth()+1;
	if(month==13){
		month=1;
	}
		...
		if(weekvos!=null&&weekvos.length>0){
			for(PaibanWeekVO weekvo:weekvos){
				...
				if(weekvo.getVbillstatus3()!=null){
					...
					if( weekvo.getVbillstatus1()!=null && "4".equals(weekvo.getVbillstatus2()) && "4".equals(weekvo.getVbillstatus1())==false){
						weekvo.setVbillstatus("门办OA提交"); //lzch
					}else if("4".equals(weekvo.getVbillstatus1())==true && weekvo.getVbillstatus2()==null){
						weekvo.setVbillstatus(weekvo.getVbillstatus()+"门办OA提交"); //lzch
					}else if("4".equals(weekvo.getVbillstatus1())==true){
						weekvo.setVbillstatus(weekvo.getVbillstatus()+"OA提交"); //lzch
					} 
...
//					weekvo.setNsygxs(map.get(bvo.getPk_psndoc()));
		...
		for(int i=0;i<list_vo.size();i++){
			PaibanWeekVO paibanweekvo =(PaibanWeekVO) list_vo.get(i);
...
//									 调入调出时间判断
									...
									if(bvos1[0].getDenddate() != null){
										if(bvos1[0].getDstartdate().compareTo(showdate)<=0
											...
											String vdate2 = paibanweekvo.getVdate();
											String beginDate = vdate2.substring(0, 10);
											UFDate dateAfter = new UFDate(beginDate).getDateAfter(j-1);
											String dateAfterString = dateAfter.toString();
											if(new UFDate(beginDate).getMonth()==dateAfter.getMonth()){
												errorMsg.append(dateAfterString+","+getPsnName(pk_psndoc)+" 漏排班\n");
											}
											//errorMsg.append(paibanweekvo.getVdate()+","+getPsnName(pk_psndoc)+" 漏排班\n");
