 ...
 *
 */
public class EventHandler5 extends CardEventHandler {
	 ...
	 * @param control
	 */
	public EventHandler5(BillCardUI billUI, ICardController control) {
...
//		}
		...
		((ClientUI5)getBillUI()).setEditstate(1);
		...
		getBillCardPanelWrapper().getBillCardPanel().getHeadItem("classtype").setEnabled(false);
		...
		int days1 = end.getDay()-begin.getDay()+1;
...
//					sqlwhere2 = " and pk_bb in (select pk_bclbid from tbm_bclb where (lbbm  like '50%' or lbbm  like '99%' or lbbm  like '30%')  and dr =0)  ";
			...
			if(classtype != null){
				if(classtype.equals("1")){
					weekvos = getWeekVOS(begindate1, enddate1,days,formLikeSQL("pk_deptzb", list)+sqlwhere1,sqlwhere2);
				}else
				{
	 ...
	 public  String[] formInSQLs(String fieldname, ArrayList fieldvalue)
	  {
	    ...
	    else
	    {
	      ...
	      {
	        sbSQL = new StringBuffer();
	        sbSQL.append("  ( ").append(fieldname).append(" IN ( ");
...
//		ArrayList<String> list_zb = new ArrayList<String>();
	...
	public PaibanWeekVO[] getBbWeekVOS(String begindate,String enddate,String sqlwhere1,String sqlwhere2) throws BusinessException{
		IUAPQueryBS service = (IUAPQueryBS)NCLocator.getInstance().lookup(IUAPQueryBS.class.getName());
		String strSQL = "select distinct pk_dept,pk_psndoc,pk_deptzb,vbillstatus3,VBILLSTATUS2,VBILLSTATUS1,pk_temp,uploadnum from trtam_paiban_b where  ddate >='"+begindate+"' and ddate<='"+enddate+"' and biszb='Y' and dr=0 " + sqlwhere1+ sqlwhere2;
		...
		if (o1.size() > 0 && o1 != null) {
			...
			for (int i = 0; i < o1.size(); i++) {
				...
				String pk_dept = new String(((Vector) o1.elementAt(i)).elementAt(0) != null ? ((Vector) o1.elementAt(i)).elementAt(0).toString() : null);
				String pk_psndoc = new String(((Vector) o1.elementAt(i)).elementAt(1) != null ? ((Vector) o1.elementAt(i)).elementAt(1).toString() : null);
				String pk_deptzb = new String(((Vector) o1.elementAt(i)).elementAt(2) != null ? ((Vector) o1.elementAt(i)).elementAt(2).toString() : null);
	...
	public PanbanWeekBVO[] getZBdetailVOS(String begindate,String enddate,String sqlwhere1,String sqlwhere2) throws BusinessException{
		IUAPQueryBS service = (IUAPQueryBS)NCLocator.getInstance().lookup(IUAPQueryBS.class.getName());
		String strSQL = "select a.ddate,a.pk_psndoc,b.lbmc from trtam_paiban_b a left join tbm_bclb b on a.pk_bb=b.pk_bclbid where ddate >='"+begindate+"' and ddate<='"+enddate+"' and biszb='Y' "+sqlwhere1+ sqlwhere2;
...
//		}
	...
	protected void onBoElse(int intBtn) throws Exception {
		// TODO Auto-generated method stub
		int rowcount = ((ClientUI5)getBillUI()).getBillCardPanel().getRowCount();

		switch (intBtn) {
		case 801:
			
			for(int i=0;i<rowcount;i++){
				((ClientUI5)getBillUI()).getBillCardPanel().getBillModel().setValueAt(false, i, "flag");
			}
			break;
		case 802:
			 rowcount = ((ClientUI5)getBillUI()).getBillCardPanel().getRowCount();
			for(int i=0;i<rowcount;i++){
				((ClientUI5)getBillUI()).getBillCardPanel().getBillModel().setValueAt(true, i, "flag");
			}
			break;
		case 803:
			onBoAudit();
