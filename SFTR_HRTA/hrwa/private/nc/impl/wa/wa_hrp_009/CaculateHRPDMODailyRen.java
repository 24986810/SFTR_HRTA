 ...
 *
 */
public class CaculateHRPDMODailyRen extends DataManageObject {


	private IHrPara hrpara;
	public CaculateHRPDMODailyRen(String arg0) throws NamingException {
		super(arg0);
	}

	public CaculateHRPDMODailyRen() throws NamingException {
		super();
	}
	//sqt
	...
	public void reCaculateHRP(ReCacuVO aRecaVO) throws BusinessException{
		...
		for(int i=0;i<hrpItems.length;i++){
			if(hrpItems[i][1].indexOf("valueofNQqDays")>=0){
	...
	public void reCaculateHRP_kkdays(ReCacuVO aRecaVO,String[][] hrpItems ) throws BusinessException{
		try {
			...
			// 员工累计工作年限,缺勤天数+旷工天数>0为请假，小于当前期间，请假合计，
			String begindate = vyear + "-01" + "-01";
			String enddate = vyear + "-" + vperiod + "-01";
			HashMap<String, UFDouble> map_daily = new HashMap<String, UFDouble>();
			
			map_daily = getDailyCount(begindate,enddate);

			BaseDAO dao = new BaseDAO();
			for (int i = 0; i < datavos.length; i++) {
					
				// 累计请假是否超3次
				UFDouble day_daily = map_daily.get(datavos[i].getPsnid());
				...
				for (int j = 0; j < hrpItems.length; j++) {
					...
					if (name.equals("ykdailyday")) {
						if(day_daily != null){
							
							if(day_daily.compareTo(new UFDouble(3.0)) >= 0){
								value = "是";
							}
	...
	public HashMap<String, UFDouble> getDailyCount(String begindate, String enddate) throws DAOException{
		HashMap<String, UFDouble> map_daily = new HashMap<String, UFDouble>();
		SuperDMO dmo = new SuperDMO();
		
		String sql = "select count(pk_psndoc) days, pk_psndoc"
			+"  from (select count(pk_bb) days, pk_psndoc, substr(ddate, 0, 7) month"
			+"          from trtam_paiban_b"
			+"         where pk_bb in ('10028L100000000002BQ',"
			+"                         '10028L100000000002C8',"
			+"                         '10028L100000000002CQ',"
			+"                         '10028L100000000002DQ',"
			+"                         '10028L100000000002F3',"
			+"                         '10028L10000000000308','10028L100000000002HZ','10028L1000000000039V')"
			+"           and ddate >= '"+begindate+"'"
			+"           and ddate < '"+enddate+"'"
			+"         group by pk_psndoc, substr(ddate, 0, 7))"
			+" group by pk_psndoc";
		Vector o1 = (Vector)dmo.executeQuery(sql, new VectorProcessor());
		if(o1.size() > 0 && o1 != null){
			for (int i = 0; i < o1.size(); i++) {
				String days = new String(((Vector) o1.elementAt(i)).elementAt(0) != null ? ((Vector) o1.elementAt(i)).elementAt(0).toString() : "");
				String pk_psndoc = new String(((Vector) o1.elementAt(i)).elementAt(1) != null ? ((Vector) o1.elementAt(i)).elementAt(1).toString() : "");
				
				map_daily.put(pk_psndoc, new UFDouble(days));
			}
		}
