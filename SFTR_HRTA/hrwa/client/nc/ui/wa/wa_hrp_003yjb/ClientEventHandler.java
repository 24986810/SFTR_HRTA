	...
	private void onBoQueryDept() throws Exception{
		...
		if(((ClientUI)getBillUI()).isListPanelSelected()){
			...
			if(row>=0){
				...
				flag = ((ClientUI)getBillUI()).getBillListPanel().getHeadBillModel().getValueAt(row, "pk_item14")!=null&&Integer.parseInt(((ClientUI)getBillUI()).getBillListPanel().getHeadBillModel().getValueAt(row, "pk_item14").toString())==WaHrpBillStatus.PASS;
			}
		}else{
			...
			flag = ((ClientUI)getBillUI()).getBillCardPanel().getHeadItem("pk_item14").getValueObject()!=null&&Integer.parseInt(((ClientUI)getBillUI()).getBillCardPanel().getHeadItem("pk_item14").getValueObject().toString())==WaHrpBillStatus.PASS;
		}
			...
			for(int i=0;i<rowcount;i++){
				try{
					...
					boolean select = ((ClientUI)getBillUI()).getBillListPanel().getHeadBillModel().getValueAt(i, "bisselect")!=null?
							...
							if(select&&hvo.getPk_item14() == WaHrpBillStatus.PASS && (hvo.getBispay()==null||!hvo.getBispay().booleanValue())){
...
//								list.add(hvo);
			...
			SuperVO[] bvos = HYPubBO_Client.queryByCondition(PsnClassItemBVO.class, " isnull(dr,0)=0 and isnull(biscount,'N')='N' and pk_psn_item_h='"+hvo.getPrimaryKey()+"' ");
			if(bvos!=null&&bvos.length>0){
				MessageDialog.showErrorDlg(this.getBillUI(), "提示","单据"+hvo.getVbillno()+"支付失败:有未复核计税的员工，请检查");
				return;
			}
			//if(hvo.getVbillstatus_audit()==WaHrpBillStatus.PASS){
			if(hvo.getPk_item14()==WaHrpBillStatus.PASS){
				hvo.setBispay(new UFBoolean(true));
				hvo.setDpaydate(_getDate());
				hvo.setVpaypsnid(_getOperator());
				hvo.setVbatchcode_approve(batchcode);
	...
	private void onCommitReturn() throws Exception{
		...
		int pk_item14 = WaHrpBillStatus.NOPASS_RETURN;
		PsnClassItemHVO hvo = (PsnClassItemHVO)getBufferData().getCurrentVO().getParentVO();
		if(hvo.getVbillstatus_audit() != null && (hvo.getVbillstatus_audit() == 2 || hvo.getVbillstatus_audit() == 3 || hvo.getVbillstatus_audit() == 4)){
			MessageDialog.showErrorDlg(this.getBillUI(), "驳回失败","奖金二级审核已审核，您不能进行驳回操作！");
			return;
		}
		...
		hvo.setVapprovenote("规培生奖金审核：" + text);
		hvo.setPk_item14(pk_item14); //一级半驳回态
		HYPubBO_Client.update(hvo);
	...
	protected void onBoCancelAudit() throws Exception {
		...
		//一级半审核状态（规培生奖金审核）
		int pk_item14 = WaHrpBillStatus.NOPASS;
		boolean islist = ((ClientUI)getBillUI()).isListPanelSelected();
		String msg = "";
		if(islist){
			int rowcount = ((ClientUI)getBillUI()).getBillListPanel().getHeadTable().getRowCount();
...
//							}
//							String rmsg = WaPubCount.onAuditYJBSH( hvo,null,pvalue,status_audit,"不通过",((ClientUI)getBillUI()).getWaPeriodVO(),false);
//							msg+=rmsg;
//							PsnClassItemHVO newhvo = (PsnClassItemHVO)HYPubBO_Client.queryByPrimaryKey(PsnClassItemHVO.class, hvo.getPrimaryKey());
//							value.setParentVO(newhvo);
...
//						}
				if(select){
					String rmsg = WaPubCount.onAuditYJBSH(hvo, null, pvalue, pk_item14, "不通过", ((ClientUI)getBillUI()).getWaPeriodVO(), false);
					msg += rmsg;
					PsnClassItemHVO newhvo = (PsnClassItemHVO)HYPubBO_Client.queryByPrimaryKey(PsnClassItemHVO.class, hvo.getPrimaryKey());
					value.setParentVO(newhvo);
					PsnClassItemBVO[] newbvos = (PsnClassItemBVO[])HYPubBO_Client.queryByCondition(PsnClassItemBVO.class, " isnull(dr,0)=0 and pk_psn_item_h='"+hvo.getPrimaryKey()+"' ");
					...
					if(newbvos != null){
						for(PsnClassItemBVO vo : newbvos){
					...
					if(list != null && !list.isEmpty()){
						value.setChildrenVO(list.toArray(new PsnClassItemBVO[0]));
					}
					getBufferData().setVOAt(i, value);
				}
			}
...
//			}
			...
			//String rmsg = WaPubCount.onAuditYJBSH(hvo, list!=null&&list.size()>0?list.toArray(new PsnClassItemBVO[0]):null, pvalue, status_audit, "不通过", ((ClientUI)getBillUI()).getWaPeriodVO(), false);
			String rmsg = WaPubCount.onAuditYJBSH(hvo, list!=null&&list.size()>0?list.toArray(new PsnClassItemBVO[0]):null, pvalue, pk_item14, "不通过", ((ClientUI)getBillUI()).getWaPeriodVO(), false);
	...
	public void onBoAudit() throws Exception {
		...
		//一级半审核状态（规培生奖金审核）
		int pk_item14 = WaHrpBillStatus.PASS;
		boolean islist = ((ClientUI)getBillUI()).isListPanelSelected();
		String msg = "";
		if(islist){
			int rowcount = ((ClientUI)getBillUI()).getBillListPanel().getHeadTable().getRowCount();
			for(int i=0;i<rowcount;i++){
				...
				boolean select = ((ClientUI)getBillUI()).getBillListPanel().getHeadBillModel().getValueAt(i, "bisselect")!=null?
						new UFBoolean(((ClientUI)getBillUI()).getBillListPanel().getHeadBillModel().getValueAt(i, "bisselect").toString()).booleanValue():false;
				if(select){
					//String rmsg = WaPubCount.onAuditYJBSH(hvo, null, pvalue, status_audit, "通过", ((ClientUI)getBillUI()).getWaPeriodVO(), true);
					String rmsg = WaPubCount.onAuditYJBSH(hvo, null, pvalue, pk_item14, "通过", ((ClientUI)getBillUI()).getWaPeriodVO(), true);
					msg += rmsg;
					PsnClassItemHVO newhvo = (PsnClassItemHVO)HYPubBO_Client.queryByPrimaryKey(PsnClassItemHVO.class, hvo.getPrimaryKey());
					value.setParentVO(newhvo);
					PsnClassItemBVO[] newbvos = (PsnClassItemBVO[])HYPubBO_Client.queryByCondition(PsnClassItemBVO.class, " isnull(dr,0)=0 and pk_psn_item_h='"+hvo.getPrimaryKey()+"' ");
			...
			//String rmsg = WaPubCount.onAuditYJBSH(hvo,list!=null&&list.size()>0?list.toArray(new PsnClassItemBVO[0]):null, pvalue, status_audit, "通过", ((ClientUI)getBillUI()).getWaPeriodVO(), true);
			String rmsg = WaPubCount.onAuditYJBSH(hvo,list!=null&&list.size()>0?list.toArray(new PsnClassItemBVO[0]):null, pvalue, pk_item14, "通过", ((ClientUI)getBillUI()).getWaPeriodVO(), true);
			MessageDialog.showHintDlg(this.getBillUI(), "提示",rmsg);
			onBoRefresh();
		}
	}
	...
	protected void onBoQueryInit() throws Exception {
		...
		//一级审核通过的数据一级半才能看到，否则看不到
		wheresql+=" and pk_item12 = 2 and pk_item14 in(" + WaHrpBillStatus.COMMIT + "," + WaHrpBillStatus.PASS + "," + WaHrpBillStatus.NOPASS+ "," + WaHrpBillStatus.NEEDPASS + ") and vdef8 = '03' ";
...
//		super.onBoRefresh();
		...
		int currRow = getBufferData().getCurrentRow();
		//过滤非规培生的子表数据begin
		if(getBufferData().getVOByRowNo(currRow) != null){
			PsnClassItemBVO[] vos = (PsnClassItemBVO[]) getBufferData().getVOByRowNo(currRow).getChildrenVO();
			List<PsnClassItemBVO> list = new ArrayList<PsnClassItemBVO>();
			if(vos != null){
			...
			if(list != null && !list.isEmpty()){
				getBufferData().getVOByRowNo(currRow).setChildrenVO(list.toArray(new PsnClassItemBVO[0]));
			}
		}
		//过滤非规培生的子表数据end
		getBufferData().setCurrentRow(currRow);
	...
	protected void onBoDel() throws Exception {
		if(getBufferData().getCurrentVO()!=null&&getBufferData().getCurrentVO().getParentVO()!=null){
			...
			hvo.setPk_item14(WaHrpBillStatus.DEL);
