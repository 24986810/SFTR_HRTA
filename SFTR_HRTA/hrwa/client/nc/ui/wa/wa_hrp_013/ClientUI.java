	...
	protected void onTreeSelectSetButtonState(TableTreeNode arg0) {
		...
		try{
			...
			if(vflag.equals(IConstant.RoleFlag)){//角色
				sql = " pk_role = '"+arg0.getNodeID().toString()+"' and pk_corp = '"+RefStrategy.getRefPK()+"' ";
			}
			if(vflag.equals(IConstant.UserFlag)){//用户
				sql = " pk_user = '"+arg0.getNodeID().toString()+"' and pk_corp = '"+RefStrategy.getRefPK()+"' ";
			}
			//判断 角色 和用户
			BusinessDelegator business = new BusinessDelegator();
			UserDeptVO[] authVOs = (UserDeptVO[])business.queryByCondition(UserDeptVO.class,sql);
			
			int nflag = this.getBillCardPanel().getRowCount();
			for (int i = 0; i < nflag; i++) {
				this.getBillCardPanel().setBodyValueAt(false, i, "flag");
				String code = this.getBillCardPanel().getBodyValueAt(i,"pk_perioddept").toString();
				for(int j = 0 ; j < authVOs.length;j++){
					if(code.equals(authVOs[j].getPk_deptdoc())&&getRefModule().getRefPK().equals(authVOs[j].getPk_module())){
