...
public class ClientUI extends HRPManagerUI{
	...
	public ClientUI() {
		...
		ClientEventHandler myeventhandler = (ClientEventHandler) createEventHandler();
		try {
			myeventhandler.onBoQueryInit();
		} catch (Exception e) {
		...
		if(getBufferData().getCurrentVO()!=null&&getBufferData().getCurrentVO().getParentVO()!=null){
			...
			if(!iscan){
				...
				if(isListPanelSelected()){
					...
					//当一级半或二级已审核（通过、不通过、驳回，则一级半不能进去审核/取消审核/驳回等操作）
					if(pk_item14 == 2 || pk_item14 == 3 || pk_item14 == 4 || status_audit == 2 || status_audit == 3 || status_audit == 4){
						...
						getButtonManager().getButton(IHRPBtn.COMMITREURN).setEnabled(false);      //驳回
					}
					if(pk_item12 == 2){
						getButtonManager().getButton(IBillButton.Audit).setEnabled(false);        //审核
