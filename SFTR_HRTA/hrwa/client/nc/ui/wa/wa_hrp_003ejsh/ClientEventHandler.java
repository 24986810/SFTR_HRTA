...
//								list.add(hvo);
			...
			if((okmsg+msg).trim().length()<=0){
				MessageDialog.showErrorDlg(this.getBillUI(), "提示","没有需要支付的单据");
			}else{
	...
	private void onCommitReturn() throws Exception{
		...
		PsnClassItemHVO hvo = (PsnClassItemHVO)getBufferData().getCurrentVO().getParentVO();
		if(!(hvo.getPk_item12() != null && hvo.getPk_item12() == 2)){
			MessageDialog.showErrorDlg(this.getBillUI(), "驳回失败","一级还未审核通过，您不能进行驳回操作！");
			return;
		}else{
			if(hvo.getVdef8() != null && "03".equals(hvo.getVdef8())){
				if(!(hvo.getPk_item14() != null && hvo.getPk_item14() == 2)){
					MessageDialog.showErrorDlg(this.getBillUI(), "驳回失败","一级半还未审核通过，您不能进行驳回操作！");
					return;
				}
			}			
		}
		...
		hvo.setVapprovenote("奖金二级审核：" + text);
		//hvo.setVapprovenote(dlg.getField_auditnode().getFieldText());
		hvo.setVbillstatus_audit(status_audit); //二级驳回态
		HYPubBO_Client.update(hvo);
...
//							}
//							String rmsg = WaPubCount.onAuditEJSH( hvo,null,pvalue,status_audit,"不通过",((ClientUI)getBillUI()).getWaPeriodVO(),false);
//							msg+=rmsg;
//							PsnClassItemHVO newhvo = (PsnClassItemHVO)HYPubBO_Client.queryByPrimaryKey(PsnClassItemHVO.class, hvo.getPrimaryKey());
//							value.setParentVO(newhvo);
...
//			}
			...
			String rmsg = WaPubCount.onAuditEJSH( hvo,list!=null&&list.size()>0?list.toArray(new PsnClassItemBVO[0]):null,pvalue,status_audit,"不通过",((ClientUI)getBillUI()).getWaPeriodVO(),false);
		...
		if(islist){
			...
			for(int i=0;i<rowcount;i++){
				...
				boolean select = ((ClientUI)getBillUI()).getBillListPanel().getHeadBillModel().getValueAt(i, "bisselect")!=null?
						new UFBoolean(((ClientUI)getBillUI()).getBillListPanel().getHeadBillModel().getValueAt(i, "bisselect").toString()).booleanValue():false;
						if(select){
							String rmsg = WaPubCount.onAuditEJSH( hvo,null,pvalue,status_audit,"通过",((ClientUI)getBillUI()).getWaPeriodVO(),true);
							msg+=rmsg;
							PsnClassItemHVO newhvo = (PsnClassItemHVO)HYPubBO_Client.queryByPrimaryKey(PsnClassItemHVO.class, hvo.getPrimaryKey());
							value.setParentVO(newhvo);
							PsnClassItemBVO[] newbvos = (PsnClassItemBVO[])HYPubBO_Client.queryByCondition(PsnClassItemBVO.class, " isnull(dr,0)=0 and pk_psn_item_h='"+hvo.getPrimaryKey()+"' ");
			...
			String rmsg = WaPubCount.onAuditEJSH( hvo,list!=null&&list.size()>0?list.toArray(new PsnClassItemBVO[0]):null,pvalue,status_audit,"通过",((ClientUI)getBillUI()).getWaPeriodVO(),true);
			MessageDialog.showHintDlg(this.getBillUI(), "提示",rmsg);
			onBoRefresh();
		}
	}
...
//		String wheresql = strWhere.toString()+" and pk_dept in ("+pk_deptid+") ";
		...
		SuperVO[] queryVos = queryHeadVOs(wheresql+" "+waclssSql+ " order by vapprovenote asc");
		...
		Collections.sort(list, new SortByBillStatus());
		getBufferData().clear();
		// 增加数据到Buffer
		addDataToBuffer(list.toArray(new SuperVO[list.size()]));
